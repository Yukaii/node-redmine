/*!
 * redmine-js
 * A JavaScript library which supports 100% features of Redmine's REST API.
...
var debug = require('debug')('redmine-js');
...
Redmine.prototype.request = function(method, path, params, callback) {
  ...
  if (this.apiKey) {
    ...
    opts.headers.Authorization = Base64.encode(opts.auth);
  } else {
  ...
  if ('PUT' == method || 'POST' == method) {
    ...
    opts.headers['Content-Type'] = this.config.format == 'json' ? 'application/json' : 'application/xml';
  }
  ...
  if ('PUT' == method || 'POST' == method) {
    option.body = opts.body;
  }

  fetch(opts.host + opts.path, option).then(function(response) {
    if (res.status != 200 && res.status != 201) {
      callback('Server returns : ' + res.statusText + ' (' + String(res.status) + ')', null);
      callback = null;
      return ;
    }
